...
- **Live updates:** the app subscribes to Supabase Realtime for the
  ...
  the worst case.
- **Calendar export:** "⬇ Download .ics" gives a one-time snapshot file.
  "🔗 Subscribe" gives a live feed URL (`/api/feed.ics`) that calendar apps
  (Apple Calendar, Google Calendar, Outlook, etc.) can subscribe to — they
  re-check it on their own schedule (typically every few hours) and pick up
  new/edited/deleted events automatically, no re-downloading required.
...
app.js                   All app logic: rendering, Supabase reads/writes,
                          the add/edit/delete modal, .ics export & subscribe
                          modal. Generic — reads everything state-specific
                          from config.js.
ics.js                   Shared iCalendar formatting used by both app.js's
                          "Download .ics" button and api/feed.js's live
                          feed, so the two can't drift apart. Generic.
api/feed.js              Serverless function: regenerates the .ics feed from
                          Supabase on every request. Served at /api/feed.ics
                          (see vercel.json). Generic — no per-instance edits.
vercel.json              Rewrites /api/feed.ics to the api/feed.js function.
