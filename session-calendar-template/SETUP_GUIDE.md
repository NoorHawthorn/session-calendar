...
5. Drag in all the files from this project folder: `index.html`, `app.js`,
   `config.js`, `README.md`, `SETUP_GUIDE.md`, and the `supabase` folder
   (with `schema.sql` and `seed_montana.sql` inside it). GitHub's
   drag-and-drop upload preserves folder structure if you drag the whole
   `supabase` folder in.
